...
const addMember = TryCatch(async (req, res, next) => {
  ...
  if (!chat) return next(new ErrorHandler(`Couldn't find chat`, 404));
  ...
  const allNewMembersProises = members.map((i) =>
    userModel.findById(i, "name")
  );

  const allNewMembers = await Promise.all(allNewMembersProises);
